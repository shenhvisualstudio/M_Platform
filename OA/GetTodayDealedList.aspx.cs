    ...
    public partial class GetTodayDealedList : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                if (strLogogram == null)
                {
                    Json = JsonConvert.SerializeObject(new DicPackage(false, null, "参数错误，获取今日已办列表失败！").DicInfo());
                    return;
            ...
            {
                Json = JsonConvert.SerializeObject(new DicPackage(false, null, string.Format("{0}：获取今日已办列表数据发生异常。{1}", ex.Source, ex.Message)).DicInfo());
            }
        }
