...
namespace ServiceInterface.Common
{
    public class InterfaceTool
    {
        ...
        /// <param name="Request">Http请求对象</param>
        /// <returns></returns>
        public static bool IdentityVerifyForSys(HttpRequest Request)
        {
            //应用名称
            string strSysName = Request.Params["SysName"];
            //签名
            string strSign = Request.Params["Sign"];

            if (string.IsNullOrWhiteSpace(strSysName) || string.IsNullOrWhiteSpace(strSign))
            {
                return false;
            }
