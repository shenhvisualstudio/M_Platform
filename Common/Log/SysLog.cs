...
namespace ServiceInterface.Common
{
    ...
    public class SysLog
    {
        ...
        //系统名称（简拼）
        public string strSysName { get; set; }
        //设备类型
        ...
        /// 初始化系统日志数据
        /// </summary>
        public SysLog(HttpRequest Request)
        {
            strCodeUser = Request.Params["CodeUser"];
            strSysName = Request.Params["SysName"];
        ...
        {
            string strSql =
                    string.Format(@"insert into TB_SYS_LOG (code_user,sysname,devicetype,ip,behavior,result,remark,behaviorurl,account)
                                    values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')",
                                    strCodeUser, strSysName, strDeviceType, strIP, strBehavior, "成功", strRemark, strBehaviorURL, strAccount);
        ...
        public void LogCatalogSuccess()
        {
            string strSql =
                    string.Format(@"insert into TB_SYS_LOG (code_user,sysname,devicetype,ip,behavior,result,remark,behaviorurl,account)
                                    values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')",
                                    strCodeUser, strSysName, strDeviceType, strIP, strBehavior, "成功", string.Empty, strBehaviorURL, strAccount);
            new Leo.Oracle.DataAccess(RegistryKey.KeyPathMa).ExecuteNonQuery(strSql);
        }
        #endregion
        ...
        public void LogCatalogFailure(string strReason)
        {
            string strSql =
                    string.Format(@"insert into TB_SYS_LOG (code_user,sysname,devicetype,ip,behavior,result,remark,behaviorurl,account)
                                    values('{0}','{1}','{2}','{3}','{4}','{5}','{6}','{7}','{8}')",
                                    strCodeUser, strSysName, strDeviceType, strIP, strBehavior, "失败", strReason, strBehaviorURL, strAccount);
            new Leo.Oracle.DataAccess(RegistryKey.KeyPathMa).ExecuteNonQuery(strSql);
        }
        #endregion
        #endregion
